...
export class UserGatewayAdapter implements UserGatewayPort {
    async create(nombres: string, apellidos: string, email: string, telefono: string, status: boolean, area: string, user_name: string, password: string, userCreated: string): Promise<any> {
        try {
            const response = await fetch(`${env.URL_USER}`, {
                ...
                body: JSON.stringify({ nombres, apellidos, email, telefono, status, area, user_name, password, userCreated }),
            });
    ...
    async update(codigo: string, nombres: string, apellidos: string, email: string, telefono: string, status: boolean, area: string, user_name: string, password: string, userUpdated: string): Promise<any> {
        ...
        try {
            const response = await fetch(`${env.URL_USER}`, {
                ...
                body: JSON.stringify({ codigo, nombres, apellidos, email, telefono, status, area, user_name, password, userUpdated }),
            });
            return await response.json();
        } catch (error) {
            console.error('Error connecting to user service:', error);
